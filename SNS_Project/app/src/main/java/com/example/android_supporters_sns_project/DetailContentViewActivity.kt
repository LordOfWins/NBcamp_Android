...
class DetailContentViewActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        when (intData) {
            1 -> {
                ...
                txtContent.text =
                    "vertical, horizontal로 2가지 가이드 라인을 만든다. 이 두 가이드 라인은 퍼센트를 활용하여 만들었기 때문에 어떤 화면에서든 같은 비율로 존재한다. 이를 바탕으로 constraint를 지정해주면 반응형 UI를 만들 수 있다. "
            }
            ...
            3 -> {
                ...
                txtContent.text = "진짜 재밌습니다... 무조건 보세요!"
            }
    ...
    private fun seeMoreView(txtContent: TextView, txtSeeMore: TextView) {
        txtContent.post {
            ...
            if (lineCount > 0 && txtContent.layout.getEllipsisCount(lineCount - 1) > 0) {
