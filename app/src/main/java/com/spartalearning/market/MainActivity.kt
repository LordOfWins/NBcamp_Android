...
import com.spartalearning.market.databinding.ActivityMainBinding
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        dataList.add(
            SaleItem(
                ...
                "울산 동해바다뷰 60평 복층 펜트하우스 1일 숙박권\\n(에어컨이 없기에 낮은 가격으로 변경했으며 8월 초 가장 더운날 다녀가신 분 경우 " +
        ...
        adapter.itemLongClick = object : ItemAdapter.ItemLongClick {
            override fun onLongClick(view: View, position: Int) {
                ...
                ad.setPositiveButton("예") { _, _ ->
                    dataList.removeAt(position)
                    adapter.notifyItemRemoved(position)
                }
                ad.setNegativeButton("아니오") { dialog, _ ->
                    dialog.dismiss()
                }
                ad.show()
            }
        ...
        binding.recyclerView.addOnScrollListener(object : RecyclerView.OnScrollListener() {
            override fun onScrollStateChanged(recyclerView: RecyclerView, newState: Int) {
                ...
                if (!binding.recyclerView.canScrollVertically(-1)
                    ...
                    binding.scrollUpButton.startAnimation(fadeOut)
                    binding.scrollUpButton.visibility = View.GONE
                    isTop = true
                } else {
                    if (isTop) {
                        binding.scrollUpButton.visibility = View.VISIBLE
                        binding.scrollUpButton.startAnimation(fadeIn)
                        isTop = false
                    }
                }
            }
        ...
        activityResultLauncher =
            registerForActivityResult(ActivityResultContracts.StartActivityForResult()) {
                if (it.resultCode == RESULT_OK) {
                    val index = it.data?.getIntExtra(Constants.ITEM_INDEX, -1)
                    val isLike = it.data?.getBooleanExtra("isLike", false)
